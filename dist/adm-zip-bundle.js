...
var require_utils = __commonJS({
  "node_modules/adm-zip/util/utils.js"(exports2, module2) {
    ...
    Utils2.prototype.writeFileTo = function(path, content, overwrite, attr) {
      const self = this;
    ...
    Utils2.prototype.writeFileToAsync = function(path, content, overwrite, attr, callback) {
    ...
    Utils2.prototype.findFiles = function(path) {
      const self = this;
      function findSync(dir, pattern, recursive) {
        ...
        self.fs.readdirSync(dir).forEach(function(file) {
          const path2 = pth2.join(dir, file);
          const stat = self.fs.statSync(path2);
          if (!pattern || pattern.test(path2)) {
            files.push(pth2.normalize(path2) + (stat.isDirectory() ? self.sep : ""));
          }
    ...
    Utils2.prototype.findFilesAsync = function(dir, cb) {
      ...
      self.fs.readdir(dir, function(err, list) {
        ...
        list.forEach(function(file) {
          file = pth2.join(dir, file);
          self.fs.stat(file, function(err2, stat) {
            if (err2) return cb(err2);
            if (stat) {
              results.push(pth2.normalize(file) + (stat.isDirectory() ? self.sep : ""));
              if (stat.isDirectory()) {
    ...
    Utils2.canonical = function(path) {
      if (!path) return "";
      const safeSuffix = pth2.posix.normalize("/" + path.split("\\").join("/"));
      return pth2.join(".", safeSuffix);
    };
    Utils2.zipnamefix = function(path) {
      if (!path) return "";
      const safeSuffix = pth2.posix.normalize("/" + path.split("\\").join("/"));
      return pth2.posix.join(".", safeSuffix);
    };
    ...
    Utils2.sanitize = function(prefix, name) {
      prefix = pth2.resolve(pth2.normalize(prefix));
      var parts = name.split("/");
      for (var i = 0, l = parts.length; i < l; i++) {
        var path = pth2.normalize(pth2.join(prefix, parts.slice(i, l).join(pth2.sep)));
        if (path.indexOf(prefix) === 0) {
          return path;
        }
      }
      return pth2.normalize(pth2.join(prefix, pth2.basename(name)));
    };
  ...
  "node_modules/adm-zip/util/fattr.js"(exports2, module2) {
    ...
    module2.exports = function(path, { fs }) {
      ...
      if (_path && fs.existsSync(_path)) {
        ...
        _obj.hidden = pth2.basename(_path)[0] === ".";
      } else {
  ...
  "node_modules/adm-zip/zipEntry.js"(exports2, module2) {
    ...
    module2.exports = function(options, input) {
      ...
      function readUInt64LE(buffer, offset) {
        return Utils2.readBigUInt64LE(buffer, offset);
      }
...
var require_zipFile = __commonJS({
  "node_modules/adm-zip/zipFile.js"(exports2, module2) {
    var ZipEntry2 = require_zipEntry();
    var Headers = require_headers();
    var Utils2 = require_util();
    module2.exports = function(inBuffer, options) {
      ...
      function makeTemporaryFolders() {
        ...
        for (const elem of foldersList) {
          if (!(elem in entryTable)) {
            const tempfolder = new ZipEntry2(opts);
      ...
      function readEntries() {
        ...
        for (var i = 0; i < entryList.length; i++) {
          var tmp = index, entry = new ZipEntry2(opts, inBuffer);
